...
Loader {
    ...
    function toggle(button)  {
        ...
        const target = rootItem;
        const pos = button.mapToItem(target, 0, 0);
        const mouseX = pos.x;
        const mouseY = pos.y;
        const radius = Math.max(distance(mouseX, mouseY, 0, 0), distance(mouseX, mouseY, target.width, 0),
            distance(mouseX, mouseY, 0, target.height), distance(mouseX, mouseY, target.width, target.height));
        const reveal = loader_reveal.item;
